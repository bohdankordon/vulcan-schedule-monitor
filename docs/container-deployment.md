...
Copy `.env.production.example` to the ignored `.env.production` (`cp` on Linux/macOS, `Copy-Item` on PowerShell). Fill in `POSTGRES_PASSWORD` with a unique random password and `VULCAN_MASTER_KEY` with the Base64 encoding of exactly 32 cryptographically random bytes. Both deliberately start empty and Compose rejects missing/empty values. Keep the master key stable with the database; replacing it makes existing encrypted sessions and remembered credentials unreadable. Store the environment file securely outside Git and restrict its filesystem access to the operator. Never paste it into logs or a PR.
...
`APP_PORT` defaults to 8080; Compose publishes only `127.0.0.1:${APP_PORT}:8080`. PostgreSQL 18.6 has no host port and is reachable by the app at `postgres:5432`, using database/user `schedule_monitor`. The default private Compose bridge permits outgoing provider access when explicitly enabled later. It does not isolate egress. The Compose smoke uses this same topology with a separate synthetic database and verifies all provider switches remain disabled in the running app.
