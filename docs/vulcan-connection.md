...
Before credential entry and before installing that request guard, the authenticator checks the landing page and discovered login page for the known ordinary VULCAN privacy consent screen. On an allowed top-level VULCAN page it matches the visible `Szanujemy Twoją prywatność` heading. It uses the nearest semantic dialog, or a nearest shared container anchored by the exact `Przejdź do ustawień` text when dialog semantics are absent. Within that boundary it selects exactly one visible, whitespace/NBSP-normalized `Zgadzam się` label: native buttons, button/submit inputs, role buttons, and ordinary text elements are supported. It never clicks settings or searches outside the privacy container for an accept action.

A genuinely absent heading adds no wait. A known visible heading with no safely resolvable container/action fails immediately at `COOKIE_CONSENT / UNSUPPORTED_AUTH_FLOW`. Hyperlinks (including ancestors), inert controls, and forwarding labels are rejected. Native form controls are inspected for their effective action and target: only same-page consent submission without credential fields, or a safe dialog dismissal, is accepted. No frame traversal or page-provided script evaluation is used. Clicking and waiting for the original container to disappear each have a three-second limit; removing just its heading is insufficient. The current page must remain allowlisted afterward. Default headless mode remains supported; MFA and CAPTCHA are never automated.

An authorized headless retry reported `DIRECT_LOGIN_DISCOVERY / TRANSIENT`. The code allowed unrecognized consent actions to silently continue into an overlay-blocked login-link click, which could exhaust Playwright's default 30-second timeout. That path fits the observation but does not establish its real cause. Discovery and navigation now have separate finite stages, `DIRECT_LOGIN_DISCOVERY` and `DIRECT_LOGIN_NAVIGATION`. Direct-login clicking and DOM-content-loaded waiting retain explicit 30-second limits to preserve normal navigation tolerance.
