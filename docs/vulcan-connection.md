...
Immediately before credential form submission, the authenticator establishes a clean observation boundary by clearing request observations and resetting session-capture diagnostics, ensuring stale pre-login requests cannot satisfy capture readiness while preserving any legitimate traffic that begins promptly during submission. After submission and `DOMContentLoaded`, runtime URL and interactive challenge checks (`requireAllowedPage` and `rejectInteractiveSecurity`) run as before. Because `DOMContentLoaded` is not sufficient session readiness for the Dziennik JavaScript application, the authenticator does not rely on a fixed delay; instead, production waits a bounded interval (defaulting to 15 seconds) using a multi-terminal condition-based wait (`waitForCondition`) that wakes promptly on any terminal event: a complete authenticated request containing both `X-V-RequestVerificationToken` and `X-V-AppGuid` headers alongside an allowed Referer, invalid credentials login UI, CAPTCHA, MFA, or a disallowed page. After the wait returns, authoritative security and runtime page checks (`requireAllowedPage` and `rejectInteractiveSecurity`) always execute before session capture, guaranteeing that security challenges and unsafe navigation take precedence over session capture even if complete requests were observed. If the wait expires without a complete request or early failure, authentication fails closed with `PROTOCOL_FAILURE` (or `INVALID_CREDENTIALS` if the password form remains visible). Header and cookie values remain secret and are never logged or included in diagnostics.
