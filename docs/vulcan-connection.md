...
Before credential entry and before installing that request guard, the authenticator checks the landing page and discovered login page for the known ordinary VULCAN privacy consent screen. On an allowed top-level VULCAN page, it matches the visible `Szanujemy Twoją prywatność` text and a unique visible `Zgadzam się` button within their nearest shared dialog/container. It normalizes whitespace, never selects `Przejdź do ustawień`, and does not inspect off-domain frames. An absent screen adds no wait. If present, clicking and waiting for the container to disappear each have a three-second limit. This works in the default headless mode; MFA and CAPTCHA remain unsupported and are never automated.
